...
export class ZebraComponent implements OnInit {
  ...
  ngOnInit() {
    this.pedidos = [];
    this.service.getAllZebra().subscribe(resp => {
  ...
  getDia() {
    this.service.getZebraToday().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }

  getSemana() {
    this.service.getZebraWeek().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }

  getMes() {
    this.service.getZebraMonth().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }

  getAno() {
    this.service.getZebraYear().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }
