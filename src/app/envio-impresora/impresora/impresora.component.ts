...
export class ImpresoraComponent implements OnInit {
  ...
  ngOnInit() {
    this.pedidos = [];
    this.service.getAllImpresora().subscribe(resp => {
  ...
  getDia() {
    this.service.getImpresoraToday().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }

  getSemana() {
    this.service.getImpresoraWeek().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }

  getMes() {
    this.service.getImpresoraMonth().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }

  getAno() {
    this.service.getImpresoraYear().subscribe(resp => {
      this.pedidos = resp.data;
    });
  }
